...
public class CartController {
    ...
    // 장바구니 조회
    @GetMapping
    public ResponseEntity<Map<String, Object>> getCart(@AuthenticationPrincipal CustomUserDetails userDetails) {
        try {
            // 현재 로그인된 사용자 정보 추출
            UserEntity currentUser = userDetails.getUser();
            
            CartResponseDto cartResponse = cartService.getUserCart(currentUser);
            
            Map<String, Object> result = new HashMap<>();
            result.put("success", true);
            result.put("message", "장바구니 조회 성공");
